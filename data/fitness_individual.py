...
def polyRegressionTime (cursor, i, matrixNum, dir):
    # for i in inputFile:
...
def polyRegression(cursor, i, matrixNum, numTimes, dir):
    # for i in inputFile:
    cursor.execute("SELECT fitnessConfig, fitness, fSize, firmRank, fitnessNorm, countAdd, countDrop, countExp, countSwitch, countBorrow"+
                   " FROM fitness F RIGHT JOIN count C " +
                   " ON (F.matrix = C.matrix" +
    ...
    polyRegressionHelper(df, dir, i, 'Borrowed', 'Fitness', 0,20,0,1)
    polyRegressionHelper(df, dir, i, 'TotalSize', 'Fitness',0,20,0,1)
    polyRegressionHelper(df, dir, i, 'Borrowed', 'Rank',0,20,0,50)
    polyRegressionHelper(df, dir, i, 'TotalSize', 'Rank',0,20,0,50)
    # polyRegressionHelper(df, dir, i, 'Add', 'Rank',0,,0,50)
    # polyRegressionHelper(df, dir, i, 'Drop', 'Rank')
    # polyRegressionHelper(df, dir, i, 'Switch', 'Rank')
    # polyRegressionHelper(df, dir, i, 'Borrow', 'Rank')

def polyRegressionHelper (df, dir, i, xName, yName,xmin,xmax,ymin,ymax):
    x = df[xName]
    y = df[yName]
    ...
    plt.xlabel(xName)
    plt.ylabel(yName)
    plt.savefig("result/" + dir + "/poly"+xName.lower()+"VS"+yName.lower()+ ".png")
    plt.close()

def firmSizeAndFitness(cursor,i,matrixNum, numTimes, dir):
    # for i in inputFile:
    ...
    ### graph 1 - 5
    linearRegressionHelper(df, dir,i,'Size','Fitness',0,20,0,1)
    linearRegressionHelper(df, dir, i, 'Borrowed', 'Fitness',0,20,0,1)
    # linearRegressionHelper(df, dir, i, 'Size', 'FitnessNorm')
    linearRegressionHelper(df, dir, i, 'Size', 'Rank',0,20,0,50)
    linearRegressionHelper(df, dir, i, 'Borrowed', 'Rank',0,20,0,50)
    ...
    plt.tight_layout()
    plt.savefig("result/" + dir + "/firmSizeOccurence.png")
    plt.clf()
    plt.close()
    ...
    # plt.tight_layout()
    plt.savefig("result/" + dir + "/3dSizeScatter.png")
    plt.clf()
    plt.close()
    ...
    fig.tight_layout()
    plt.savefig("result/" + dir + "/sizeHeatMap.png")
    plt.clf()
    plt.close()

def performanceIteration(cursor, f,matrixNum, iterationNum, numTimes, dir):
...
def avgRankChange(cursor, f, matrixNum,iterationNum, numTimes, dir):
    firms = range(0, 20)
    # for f in inputFile:
    meanArr = []
    stdArr = []
...
def linearRegressionHelper(df, dir, i, xName, yName,xmin,xmax,ymin,ymax):
    ...
    plt.text(df.min(axis=0)[xName], 0.9, text, wrap=True)
    # print(text)
    plt.savefig("result/" + dir + "/"+xName.lower()+"VS"+yName.lower()+ ".png")
    plt.close()
